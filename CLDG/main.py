...
def train(dataset, hidden_dim, n_layers, n_classes, fanouts, snapshots, views, strategy, readout, batch_size, num_workers, epochs, GPU):
    device_id = GPU
    ...
    for step, (input_nodes, seeds, blocks) in enumerate(test_dataloader):
        ...
        batch_embeddings = test_batch_logits.cpu().data.numpy()
        if step == 0:
    ...
    ''' Linear Evaluation '''
    labels, train_idx, val_idx, test_idx = dataloader(DATASET)
    train_embs = th.tensor(embeddings[train_idx]).to(device_id)
    val_embs = th.tensor(embeddings[val_idx]).to(device_id)
    test_embs = th.tensor(embeddings[test_idx]).to(device_id)
...
if __name__ == '__main__':
    ...
    fanouts=FANOUTS, snapshots=SNAPSHOTS, views=VIEWS, strategy=STRATEGY, readout=READOUT, batch_size=BATCH_SIZE, num_workers=WORKERS, epochs=EPOCHS,
    GPU=GPU)
